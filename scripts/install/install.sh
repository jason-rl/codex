...
Environment:
  ...
  CODEX_INSTALLER_USE_RELEASES_OPENAI_COM
                         Set to 1, true, or yes to use releases.openai.com.
EOF
        exit 0
        ;;
      *)
...
release_url_for_asset() {
  ...
  printf 'https://github.com/jason-rl/codex/releases/download/rust-v%s/%s\n' "$resolved_version" "$asset"
}
...
release_metadata_url() {
  ...
  printf 'https://api.github.com/repos/jason-rl/codex/releases/tags/rust-v%s\n' "$resolved_version"
}
...
resolve_release_from_github() {
  ...
  if [ "$normalized_version" = "latest" ]; then
    ...
    metadata_url="https://api.github.com/repos/jason-rl/codex/releases/latest"
  else
